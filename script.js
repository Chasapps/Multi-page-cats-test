...
function assignCategory(idx){
  ...
  openCategoryNavPage(idx); /* replaced modal with nav page */
...
// ===== Category NAV PAGE (full-screen) =====
function openCategoryNavPage(idx){
  try{
    const page = document.getElementById('categoryPage');
    const back = document.getElementById('catBackBtn');
    const search = document.getElementById('catSearch');
    const list = document.getElementById('catList');
    const specials = new Set(['+ Add new category...','Uncategorised']);

    // Build categories
    const fromTxns = Array.from(new Set((CURRENT_TXNS||[]).map(t => (t.category||'').trim()).filter(Boolean)));
    const fromRules = Array.from(new Set((CURRENT_RULES||[]).map(r => (r && r.category ? String(r.category) : '').trim()).filter(Boolean)));
    let base = Array.from(new Set([...fromTxns, ...fromRules]));
    base = base.map(c => (String(c).toUpperCase()==='UNCATEGORISED' ? 'Uncategorised' : c)).filter(Boolean);
    if (!base.includes('Uncategorised')) base.unshift('Uncategorised');
    base.unshift('+ Add new category...');
    ...
    function render(){
      ...
      categories
        ...
        .forEach(name => {
          ...
          div.className = 'cat-item' + (specials.has(name)?' special':'');
          div.textContent = name;
          div.onclick = ()=>{
            const lo = name.toLowerCase();
            const isAdd = name.startsWith('➕') || name.startsWith('+') || lo.indexOf('add new category') !== -1;
            if (isAdd) {
              closePage();
              assignCategory_OLD(idx);
              return;
            }
            const norm = (name === 'Uncategorised') ? '' : String(name).trim().toUpperCase();
            if (CURRENT_TXNS && CURRENT_TXNS[idx]) CURRENT_TXNS[idx].category = norm;
            try { renderMonthTotals(); } catch {}
            try { renderTransactionsTable(); } catch {}
            closePage();
    ...
    setTimeout(()=>{ try{ search.focus(); }catch{} }, 60);
    render();
  }catch(e){
    console.error('openCategoryNavPage error', e);
    alert('Category picker failed to open.');
  }
}
